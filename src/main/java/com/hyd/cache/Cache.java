...
public class Cache {
    ...
    private <T> T extractFromElement(Object value) {
        if (value instanceof Element) {
            Element<T> element = (Element<T>) value;
            if (element.expired()) {
                return null;
            } else {
     ...
     * @return 对象值
     *
     * @throws CacheException 如果缓存中的元素类型不是 Element
     */
    public <E> Element<E> getElement(String key) throws CacheException {
        Object value = this.cacheAdapter.get(key);
